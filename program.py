...
end_date: str = str(date.today()) # Today date.
...
zulip_response = requests.post("http://94.79.54.21:3000/api/zulip/getData",
                               ...
                               data=json.dumps({"studEmail": miem_email,
                                                ...
                                                "token": token,
                                                "timeRange": 1}))
# Send request to Jitsu.com ==>
jitsi_begin_date: str = "2021-09-15"
jitsi_rooms: list = [] # rooms list.
...
jitsi_2response = requests.post("http://94.79.54.21:3000/api/jitsi/sessions",
                                ...
                                data=json.dumps({"studEmail": edu_email,
                                                 ...
                                                 "endDate": end_date,
                                                 "token": token}))
# Send request to gitlab.com ==>
git_begin_date: str = "2022-01-01"
...
if zulip_response.status_code == jitsi_1response.status_code == jitsi_2response.status_code \
    ...
    # Zulip
    zulip_data: object = zulip_response.json()
    total_zulip_messages: int = len(zulip_data["messages"])
    zulip_streams_numbers: list = []
    zulip_streams_names: list = []
    ...
    fig1: object = px.bar(x=zulip_first_data.keys(),
    ...
    fig2: object = px.line(x=zulip_first_data.keys(),
                           y=accumulate,
    ...
    # Jitsi
    jitsi_data: object = jitsi_1response.json()
    jitsi_first_data: dict = {}
    
    for x in jitsi_data:
        if x["room"] not in jitsi_rooms:
            jitsi_rooms.append(x["room"])
            
        mas: list = x["date"].split("-")
        line = mas[2] + " " + mas[1] + " " + mas[0]
        minutes: int = (int(x["end"][:2]) - int(x["begin"][:2])) * 60 + (int(x["end"][3:5]) - int(x["begin"][3:5]))
        study_hours: int = minutes / 40
    ...
    jitsi_data: object = jitsi_2response.json()
    for x in jitsi_data:
        if x["room"] not in jitsi_rooms:
            jitsi_rooms.append(x["room"])
    ...
    # Taiga
    taiga_tasks_data: object = taiga_tasks_response.json()
    taiga_tasks_massive: list = []
        
    for x in taiga_tasks_data:
        if x["owner"] == 923:
            taiga_tasks_massive.append(x)

    taiga_us_data: object = taiga_us_response.json()
    taiga_us_massive: list = []
    
    for x in taiga_us_data:
        if x["owner"] == 923:
            taiga_us_massive.append(x)
            
    taiga_first_data: dict = {}
    ...
    fig5: object = px.line(x=taiga_first_data.keys(),
                   y=accum,
                   labels={"x": "Дата", "y": "Количество задач"},
                   title="Создание задач")
    total_us: int = len(taiga_us_massive)
    total_tasks: int = len(taiga_tasks_massive)
    
    # Gitlab
    git_data: object = git_response.json()
    git_first_data: dict = {}
    total_commits: int = 0
    ...
    for x in git_data["projects"]:
        ...
        for y in x["commits_stats"]:
            if y["commitCount"] != 0:
                ...
                current_date: str = line[1] + " " + line[2]
    ...
    git_first_data: dict = dict(sorted(git_first_data.items(), key=lambda x: x[0]))

    fig6: object = px.bar(x=git_first_data.keys(),
    ...
    fig7: object = px.line(x=git_first_data.keys(),
                   y=accum,
                   labels={"x": "Дата", "y": "Количество коммитов"},
                   title="Коммиты")
